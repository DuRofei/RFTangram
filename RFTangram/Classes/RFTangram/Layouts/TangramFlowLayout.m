    ...
    for (int i=0; i<maxRows; i++) {
        ...
        if (self.bgImgURL && self.bgImgURL.length > 0) {
            ...
            switch (self.bgScaleType) {
                ...
                {
                    [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:[NSURL URLWithString:self.bgImgURL] options:0 progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
                        if (image) {
